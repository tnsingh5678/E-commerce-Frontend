...
const Customers = () => {
  ...
  const fetchCustomers = async () => {
    try {
      const response = await fetch('https://e-commerce-backend-im60.onrender.com/get-user');
      const data = await response.json();
      if (data.success) {
  ...
  const handleStatusChange = async (userId, newStatus) => {
    try {
      const response = await fetch('https://e-commerce-backend-im60.onrender.com/update-account-status', {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
