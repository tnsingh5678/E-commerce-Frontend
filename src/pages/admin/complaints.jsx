...
const Complaints = () => {
  ...
  const fetchComplaints = async () => {
    try {
      const response = await fetch('https://e-commerce-backend-im60.onrender.com/complaints/get-complaints');
      const data = await response.json();
  ...
  const handleStatusChange = async (complaintId, newStatus) => {
    try {
      const response = await fetch('https://e-commerce-backend-im60.onrender.com/update-complaint-status', {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json'
        },
